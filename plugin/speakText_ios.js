...
(function() {
	...
	var audioManager = (function() {
		var AudioContext;
		if (!window.o2WrapperNativeInjected && (AudioContext = window.AudioContext || window.webkitAudioContext)) {
			var context = new AudioContext();
			var buffers = {};

			if ((browser.isIOs || browser.isAndroid) && !window.o2WrapperNativeInjected) {
		...
		return {
			loadAudio : function(audios) {
				...
				for (var key in audios) {
					audioCache[key] = new Sound();
					defers.push(audioCache[key].setFile(audios[key]));
				}
